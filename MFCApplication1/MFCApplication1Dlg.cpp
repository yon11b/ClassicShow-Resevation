...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LOGIN_ID, m_LoginId);
	DDX_Control(pDX, IDC_LOGIN_PW, m_LoginPw);
}
...
void CMFCApplication1Dlg::OnBnClickedSignup()
{
	...
	SignupDlg.Create(IDD_SIGNUP);
	SignupDlg.CenterWindow();
	SignupDlg.ShowWindow(SW_SHOW);
}
...
void CMFCApplication1Dlg::OnBnClickedLogin()
{
	...
	m_LoginId.GetWindowText(loginid);
	m_LoginPw.GetWindowText(loginpw);

	hDbc = DB.hDbc;
	if (SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt) == SQL_SUCCESS)
	{
		sprintf_s((char*)query, 101, "SELECT COUNT(*)  FROM [USER] WHERE ID='%s' AND PW='%s' ", loginid, loginpw);
...
void CMFCApplication1Dlg::OnBnClickedFin()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	MessageBox("DB CLOSE!");
	CDialog::OnCancel();
