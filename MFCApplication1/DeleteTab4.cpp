...
BOOL CDeleteTab4::OnInitDialog() {
    ...
    if (DB.db_connect()) {
        ...
        {
            sprintf_s((char*)query, 201, "SELECT REVIEWNO, DETAIL, HALLNAME, ROOMNAME, SEATNUM, SOUND, [VIEW], TOTAL FROM REVIEW R, CONCERTHALL C WHERE R.HALLNUM = C.HALLNO");
    ...
    if (DB.db_connect()) {
        ...
        if (SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt) == SQL_SUCCESS)
        {
            ...
            if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO) {
                MessageBox("리뷰 삭제에 실패했습니다.");
            }
            else {
                MessageBox("성공적으로 리뷰를 삭제했습니다.");
            }
